...
\textit{XSalsa20} is a derivation of \textit{Salsa20} to support a 192-bit nonce 
and the generation a 256-bit "subkey".  After initializing the state, which is 
identical to the Salsa20 state, the remaining XSalsa20 functions of crypt(), 
keystream(), and done() are simply use their counterparts of Salsa20.

XSalsa20 takes a 256-bit key and "hashes" it with the first 128 bits of the 
nonce using the Salsa20 doubleround function (without the final addition) to 
produce the subkey.  This subkey and the remaining 64 bits of the nonce are used 
to initialize the state.  

If you have a need to store the subkey, it can be retrieved from xsalsa20_setup() 
by providing a pointer to a 32-byte memory location.  If there is no such need 
pass a NULL.

Like Salsa20, XSalsa20 supports reduced, even-numbered rounds although 20 rounds 
is the recommendation. 

To initialize XSalsa20, key size must be 32 bytes (256 bits) and nonce size 
must be 24 bytes (192 bits).  Rounds must be an even number and if set to 0 
it will be changed to 20.  As always never ever use the same (key, nonce) pair 
more than once.

\begin{verbatim}
salsa20_state st;
err = xsalsa20_setup(&st, key, key_len, nonce, nonce_len, rounds, &subkey);
\end{verbatim}

For the actual encryption or decryption call:
\begin{verbatim}
err = xsalsa20_crypt(&st, in_buffer, in_len, out_buffer);
\end{verbatim}

If you just want a random stream of bytes, initialize the cipher with a 
random \textit{key} (32 bytes), a random \textit{nonce} (24 bytes), after 
which you will get a stream of pseudo-random bytes via:
\begin{verbatim}
err = xsalsa20_keystream(&st, out_buffer, out_len);
\end{verbatim}

When finished you should wipe the state:
\begin{verbatim}
err = xsalsa20_done(&st);
\end{verbatim}

xsalsa20_setup() is the only call needed to completely initialize the state.  
Calling xsalsa20_ivctr64() with the remaining 64 nonce bits and a counter of 
zero is redundant.

Generally speaking, use of xsalsa20_ivctr64() is not recommended.  Exercise 
extreme care if you do because it is possible to loose nonce uniqueness over 
time if the nonce's uniqueness is confined to the last 64-bits of the nonce.  
(XSalsa20 then becomes the same as Salsa20 with the exception the subkey's 
one level of indirection from the key.)
