...
int ocb_decrypt_last(ocb_state *ocb, const unsigned char *ct, unsigned long ctlen, unsigned char *pt)
{
   ...
   if (full_blocks>0) {
     if ((err = ocb_decrypt(ocb, ct, full_blocks_len, pt)) != CRYPT_OK) {
       goto LBL_ERR;
     }
   }
   ...
   if (last_block_len>0) {
     ...
     ocb_int_xor_blocks(iOffset_star, ocb->Offset_current, ocb->L_star, ocb->block_len);
     ...
     ocb_int_xor_blocks(pt+full_blocks_len, (unsigned char *)ct+full_blocks_len, iPad, last_block_len);

     /* Checksum_* = Checksum_m xor (P_* || 1 || zeros(127-bitlen(P_*))) */
     ocb_int_xor_blocks(ocb->checksum, ocb->checksum, pt+full_blocks_len, last_block_len);
