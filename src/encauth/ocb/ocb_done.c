...
int ocb_done(ocb_state *ocb, unsigned char *tag, unsigned long *taglen)
{
   ...
   if (ocb->adata_buffer_bytes>0) {
     ...
     ocb_int_xor_blocks(ocb->aOffset_current, ocb->aOffset_current, ocb->L_star, ocb->block_len);

     /* CipherInput = (A_* || 1 || zeros(127-bitlen(A_*))) xor Offset_* */
     ocb_int_xor_blocks(tmp, ocb->adata_buffer, ocb->aOffset_current, ocb->adata_buffer_bytes);
     ...
     ocb_int_xor_blocks(ocb->aSum_current, ocb->aSum_current, tmp, ocb->block_len);
   }
   ...
   /* tag = tag ^ HASH(K, A) */
   ocb_int_xor_blocks(tmp, ocb->tag_part, ocb->aSum_current, ocb->block_len);
