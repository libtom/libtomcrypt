...
int ocb_encrypt_last(ocb_state *ocb, const unsigned char *pt, unsigned long ptlen, unsigned char *ct)
{
   ...
   if (full_blocks>0) {
     if ((err = ocb_encrypt(ocb, pt, full_blocks_len, ct)) != CRYPT_OK) {
       goto LBL_ERR;
     }
   }
   ...
   if (last_block_len>0) {
     ...
     ocb_int_xor_blocks(ct+full_blocks_len, pt+full_blocks_len, iPad, last_block_len);
