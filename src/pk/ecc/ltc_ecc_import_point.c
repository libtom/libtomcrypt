...
int ltc_ecc_import_point(const unsigned char *in, unsigned long inlen, void *prime, void *a, void *b, void *x, void *y)
{
   ...
   if (ltc_mp_init_multi(&t1, &t2, NULL) != CRYPT_OK) {
      return CRYPT_MEM;
   }
   ...
   if (in[0] == 0x04 && (inlen&1) && ((inlen-1)>>1) == size) {
      ...
      /* load x */
      if ((err = ltc_mp_read_unsigned_bin(x, (unsigned char *)in+1, size)) != CRYPT_OK)      { goto cleanup; }
      /* load y */
      if ((err = ltc_mp_read_unsigned_bin(y, (unsigned char *)in+1+size, size)) != CRYPT_OK) { goto cleanup; }
   }
   ...
   err = CRYPT_OK;
cleanup:
   ltc_mp_deinit_multi(t1, t2, NULL);
   return err;
}
