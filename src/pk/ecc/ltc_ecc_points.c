...
{
   int err;
   if ((err = mp_set_int(p->x, x)) != CRYPT_OK) return err;
   if ((err = mp_set_int(p->y, y)) != CRYPT_OK) return err;
   if ((err = mp_set_int(p->z, z)) != CRYPT_OK) return err;
   return CRYPT_OK;
}
...
{
   int err;
   if ((err = mp_copy(src->x, dst->x)) != CRYPT_OK) return err;
   if ((err = mp_copy(src->y, dst->y)) != CRYPT_OK) return err;
   if ((err = mp_copy(src->z, dst->z)) != CRYPT_OK) return err;
   return CRYPT_OK;
}
