...
static int s_rsa_make_key(prng_state *prng, int wprng, int size, void *e, rsa_key *key)
{
   ...
   if ((err = ltc_mp_init_multi(&p, &q, &tmp1, &tmp2, NULL)) != CRYPT_OK) {
      return err;
   }
...
errkey:
   rsa_free(key);
cleanup:
   ltc_mp_deinit_multi(tmp2, tmp1, q, p, NULL);
   return err;
}
                        ...
                        const unsigned char *e, unsigned long elen, rsa_key *key)
{
   ...
   if ((err = ltc_mp_read_unsigned_bin(tltc_mp_e, (unsigned char *)e, elen)) == CRYPT_OK)
     err = rsa_make_key_bn_e(prng, wprng, size, tltc_mp_e, key);
