...
*/
static int _dsa_make_params(prng_state *prng, int group_size, int modulus_size, void *p, void *q, void *g)
{
...
int dsa_generate_pqg(prng_state *prng, int group_size, int modulus_size, dsa_key *key)
{
   ...
   /* generate params */
   err = _dsa_make_params(prng, group_size, modulus_size, key->p, key->q, key->g);
   if (err != CRYPT_OK) {
      goto cleanup;
   }
