...
ML-KEM and ML-DSA require \code{LTC\_SHA3} to be enabled.
SLH-DSA requires \code{LTC\_SHA3} for the SHAKE parameter sets and \code{LTC\_SHA256} plus
\code{LTC\_SHA512} for the SHA-2 ones.

None of the PQC algorithms uses the hash registry: the parameter set fixes the hash, so ML-KEM, ML-DSA and
the SLH-DSA SHAKE parameter sets call SHA-3 and SHAKE directly, and the SLH-DSA SHA-2 parameter sets use the
\code{sha256\_desc} and \code{sha512\_desc} descriptors directly.  No \code{register\_hash()} or
\code{register\_all\_hashes()} call is needed before a PQC key is generated, imported or used.
...
For ML-KEM and ML-DSA the PKCS \#8 private key is a CHOICE of the generation seed, the expanded private
key, or both (RFC 9935 and RFC 9881). The importers accept all three forms, and
\code{enum ltc\_pqc\_privkey\_format} selects which one \code{mlkem\_export\_ex()} and
\code{mldsa\_export\_ex()} write. SLH-DSA has no such choice, because its raw private key already
contains SK.seed, SK.prf and PK.seed, so storing it loses nothing.
...
The macro \code{LTC\_PQC\_CTX\_MAX\_BYTES} (= 255) is the maximum length of the optional context string
\textit{ctx} accepted by ML-DSA and SLH-DSA signing/verification (per FIPS 204 \S5.2 and FIPS 205 \S10.2).
...
Returns the key, ciphertext, shared secret, key-generation seed and encapsulation entropy sizes in bytes
for the parameter set specified by \textit{alg}.
...
The remaining ML-KEM byte-length constants are fixed across all parameter sets and are exposed as
preprocessor macros in \texttt{tomcrypt\_pqc.h}:
...
\hline
\code{LTC\_MLKEM\_M\_BYTES}             & 32 & Encaps entropy \textit{m} for \code{mlkem\_encaps\_ex()} \\
\code{LTC\_MLKEM\_SHARED\_SECRET\_BYTES} & 32 & Output of \code{mlkem\_encaps()} / \code{mlkem\_decaps()} \\
\code{LTC\_MLKEM\_KEYGEN\_SEED\_BYTES}   & 64 & Seed for \code{mlkem\_make\_key\_from\_seed()} ($d \,\|\, z$) \\
\hline
\end{tabular}
\end{center}

\index{mlkem\_find\_alg()}\index{mlkem\_alg\_name()}
\begin{verbatim}
int mlkem_find_alg(const char *name_or_oid, int *alg);
int mlkem_alg_name(int alg, const char **name);
\end{verbatim}

\code{mlkem\_find\_alg()} resolves the canonical FIPS 203 name (e.g. \texttt{"ML-KEM-768"})
or the dotted-decimal OID string (e.g. \texttt{"2.16.840.1.101.3.4.4.2"})
to an \code{ltc\_mlkem\_id} value.  Name comparison is case-insensitive and
ignores \texttt{'-'} and \texttt{'\_'}, so e.g. \texttt{"ML-KEM-768"},
\texttt{"ml\_kem\_768"}, and \texttt{"mlkem768"} all resolve identically.
\code{mlkem\_alg\_name()} performs the inverse lookup: given an \code{ltc\_mlkem\_id}, it returns a pointer
to a static name string (the caller must not free it).
Both return \code{CRYPT\_OK} on success and \code{CRYPT\_INVALID\_ARG} on miss.
...
\subsection{Generation Seed}

ML-KEM key expansion is one way: the seed cannot be computed back from the expanded private key.
A key object therefore stores the seed it was generated from, so that the compact RFC 9935
private-key form stays available.

\index{mlkem\_has\_seed()}
\begin{verbatim}
int mlkem_has_seed(const mlkem_key *key);
\end{verbatim}

Returns 1 if \textit{key} is a private key that still has its generation seed, 0 otherwise
(a \code{NULL} pointer returns 0). Keys made by \code{mlkem\_make\_key()} or
\code{mlkem\_make\_key\_from\_seed()} have one, and so do keys imported from the RFC 9935
\textit{seed} or \textit{both} form. Keys imported from an expanded private key, raw or
PKCS \#8, do not.

\index{mlkem\_export\_seed()}
\begin{verbatim}
int mlkem_export_seed(unsigned char *out, unsigned long *outlen,
                     const mlkem_key *key);
\end{verbatim}

Writes the 64-byte generation seed of a private key.
Returns \code{CRYPT\_PK\_NOT\_PRIVATE} for a public key, \code{CRYPT\_PK\_INVALID\_TYPE} if the key
has no seed, and \code{CRYPT\_BUFFER\_OVERFLOW} if the buffer is too small, with \textit{*outlen}
set to the required size.
The seed is private key material and needs the same protection as the expanded private key.
\code{mlkem\_free()} wipes the copy inside the key object.

\subsection{Key Validation}

\index{mlkem\_check\_key()}
\begin{verbatim}
int mlkem_check_key(const mlkem_key *key);
\end{verbatim}

Checks that every coefficient of the encapsulation key is below $q$, that the copy of the
encapsulation key inside the decapsulation key matches, that the stored $H(ek)$ matches, and, if the
key has a generation seed, that expanding the seed again gives the same key pair.
Since the key structure is public, this is how to check a key object that was built or changed
by hand. The importers run the same checks, so calling it on a freshly imported key adds nothing.
Returns \code{CRYPT\_OK} if the key is consistent, \code{CRYPT\_PK\_INVALID\_TYPE} if a key field
is wrong, and \code{CRYPT\_INVALID\_PACKET} if the key material does not match.
...
For \code{PK\_PRIVATE \& PK\_STD} the PKCS \#8 \textit{privateKey} holds the RFC 9935 CHOICE. It is
written as the seed if the key has one and as the expanded private key otherwise.
Use \code{mlkem\_export\_ex()} to choose the form explicitly.

\index{mlkem\_export\_ex()}
\begin{verbatim}
int mlkem_export_ex(unsigned char *out, unsigned long *outlen,
                    int which, enum ltc_pqc_privkey_format format,
                    const mlkem_key *key);
\end{verbatim}

Same as \code{mlkem\_export()}, with the private-key representation chosen by \textit{format}:

\begin{figure}[H]
\begin{center}
\begin{tabular}{|l|l|}
\hline \textbf{format} & \textbf{PK\_PRIVATE \& PK\_STD output} \\
\hline LTC\_PQC\_PRIVKEY\_AUTO     & seed if the key has one, expandedKey otherwise \\
\hline LTC\_PQC\_PRIVKEY\_SEED     & seed \\
\hline LTC\_PQC\_PRIVKEY\_EXPANDED & expandedKey \\
\hline LTC\_PQC\_PRIVKEY\_BOTH     & SEQUENCE \{ seed, expandedKey \} \\
\hline
\end{tabular}
\end{center}
\caption{Possible mlkem\_export\_ex() private-key representations}
\end{figure}

\code{LTC\_PQC\_PRIVKEY\_SEED} and \code{LTC\_PQC\_PRIVKEY\_BOTH} return
\code{CRYPT\_PK\_INVALID\_TYPE} for a key without a seed. Before \code{LTC\_PQC\_PRIVKEY\_BOTH} is
written, the seed is expanded again and compared with the key, so that the two parts of the
encoding cannot disagree.
All other values of \textit{which} need \textit{format} set to \code{LTC\_PQC\_PRIVKEY\_AUTO} and
return \code{CRYPT\_INVALID\_ARG} otherwise. In particular \code{PK\_PRIVATE} without
\code{PK\_STD} always writes the raw expanded private key.
...
Deterministic variant of \code{mlkem\_encaps()} corresponding to the
\textit{ML-KEM.Encaps\_internal} routine in FIPS 203 \S6.2.
Instead of drawing the 32-byte encapsulation entropy from a PRNG,
the caller supplies it as \textit{m} (\textit{mlen} must equal 32).
This is intended for known-answer testing, deterministic protocol variants,
and rerunning a failed encapsulation against the same input.
For ordinary use, \code{mlkem\_encaps()} should be preferred.
Returns \code{CRYPT\_INVALID\_ARG} if \textit{mlen} is not 32, or
\code{CRYPT\_BUFFER\_OVERFLOW} if either output buffer is too small.
...
Returns the public key, secret key, signature, per-signature \textit{rnd}, external \textit{mu} and
key-generation seed sizes in bytes for the parameter set specified by \textit{alg}.
Any output pointer may be \code{NULL} if the caller does not need that particular size.
Returns \code{CRYPT\_OK} on success, or \code{CRYPT\_INVALID\_ARG} if \textit{alg} is not a valid ML-DSA parameter set.

The remaining ML-DSA byte-length constants are fixed across all parameter sets and are exposed as
preprocessor macros in \texttt{tomcrypt\_pqc.h}:
...
\textbf{Macro} & \textbf{Value} & \textbf{Meaning} \\
\hline
\code{LTC\_MLDSA\_RND\_BYTES}         & 32 & Per-signature \textit{rnd} for \code{mldsa\_sign\_ex()} \\
\code{LTC\_MLDSA\_MU\_BYTES}          & 64 & External \textit{mu} for \code{mldsa\_sign\_ex\_mu()} \\
\code{LTC\_MLDSA\_KEYGEN\_SEED\_BYTES} & 32 & Seed for \code{mldsa\_make\_key\_from\_seed()} ($\xi$) \\
\hline
\end{tabular}
\end{center}

\index{mldsa\_find\_alg()}\index{mldsa\_alg\_name()}
\begin{verbatim}
int mldsa_find_alg(const char *name_or_oid, int *alg);
int mldsa_alg_name(int alg, const char **name);
\end{verbatim}

\code{mldsa\_find\_alg()} resolves the canonical FIPS 204 name (e.g. \texttt{"ML-DSA-65"})
or the dotted-decimal OID string (e.g. \texttt{"2.16.840.1.101.3.4.3.18"})
to an \code{ltc\_mldsa\_id} value.  Name comparison is case-insensitive and
ignores \texttt{'-'} and \texttt{'\_'}.
\code{mldsa\_alg\_name()} performs the inverse lookup: given an \code{ltc\_mldsa\_id}, it returns a pointer
to a static name string (the caller must not free it).
Both return \code{CRYPT\_OK} on success and \code{CRYPT\_INVALID\_ARG} on miss.
...
ML-DSA key expansion is one way: the seed cannot be computed back from the expanded private key.
A key object therefore stores the seed it was generated from, so that the compact RFC 9881
private-key form stays available.

\index{mldsa\_has\_seed()}
\begin{verbatim}
int mldsa_has_seed(const mldsa_key *key);
\end{verbatim}

Returns 1 if \textit{key} is a private key that still has its generation seed, 0 otherwise
(a \code{NULL} pointer returns 0). Keys made by \code{mldsa\_make\_key()} or
\code{mldsa\_make\_key\_from\_seed()} have one, and so do keys imported from the RFC 9881
\textit{seed} or \textit{both} form. Keys imported from an expanded private key, raw or
PKCS \#8, do not.

\index{mldsa\_export\_seed()}
\begin{verbatim}
int mldsa_export_seed(unsigned char *out, unsigned long *outlen,
                     const mldsa_key *key);
\end{verbatim}

Writes the 32-byte generation seed of a private key.
Returns \code{CRYPT\_PK\_NOT\_PRIVATE} for a public key, \code{CRYPT\_PK\_INVALID\_TYPE} if the key
has no seed, and \code{CRYPT\_BUFFER\_OVERFLOW} if the buffer is too small, with \textit{*outlen}
set to the required size.
The seed is private key material and needs the same protection as the expanded private key.
\code{mldsa\_free()} wipes the copy inside the key object.

\subsection{Key Validation}

\index{mldsa\_check\_key()}
\begin{verbatim}
int mldsa_check_key(const mldsa_key *key);
\end{verbatim}

Rebuilds the public key from the signing key, compares it with the stored public key, checks the
encoded $t_0$ and $tr$ against it, and, if the key has a generation seed, checks that expanding the
seed again gives the same key pair.
Since the key structure is public, this is how to check a key object that was built or changed
by hand. The importers run the same checks, so calling it on a freshly imported key adds nothing.
Returns \code{CRYPT\_OK} if the key is consistent, \code{CRYPT\_PK\_INVALID\_TYPE} if a key field
is wrong, and \code{CRYPT\_INVALID\_PACKET} if the key material does not match.
...
The standard formats are DER encoded.
For \code{PK\_PRIVATE \& PK\_STD} the PKCS \#8 \textit{privateKey} holds the RFC 9881 CHOICE. It is
written as the seed if the key has one and as the expanded private key otherwise.
Use \code{mldsa\_export\_ex()} to choose the form explicitly.

\index{mldsa\_export\_ex()}
\begin{verbatim}
...
Same as \code{mldsa\_export()}, with the private-key representation chosen by \textit{format}:
...
\end{tabular}
\end{center}
\caption{Possible mldsa\_export\_ex() private-key representations}
\end{figure}
...
or \code{CRYPT\_INVALID\_PACKET} if the private key encodes \textit{s1}/\textit{s2}
coefficients outside $[-\eta, +\eta]$ as required by FIPS 204 \S3.6.3.

\index{mldsa\_sign\_ex()}
\begin{verbatim}
...
Deterministic variant of \code{mldsa\_sign()} corresponding to the
\textit{ML-DSA.Sign\_internal} routine in FIPS 204 \S5.2.
Instead of drawing the 32-byte per-signature randomness $\rho'$ from a PRNG,
the caller supplies it as \textit{rnd} (\textit{rndlen} must equal 32).
Setting \textit{rnd} to all zeros yields the spec's ``deterministic'' variant;
setting it from a CSPRNG yields the ``hedged'' variant produced by
\code{mldsa\_sign()}.
This is required to verify FIPS 204 known-answer test vectors, which fix \textit{rnd}.
Returns \code{CRYPT\_INVALID\_ARG} if \textit{rndlen} is not 32, and the same
status codes as \code{mldsa\_sign()} otherwise.

\index{mldsa\_sign\_ex\_mu()}
\begin{verbatim}
...
Signs using a precomputed message hash, corresponding to the
\textit{ML-DSA.Sign\_internal} signing path in FIPS 204 \S5.4.1 with
externally supplied $\mu$.
The 64-byte \textit{mu} is the message-binding hash $\mu = \mathrm{SHAKE256}(\mathrm{BytesToBits}(tr) \,\|\, M', 64)$;
\textit{mulen} must equal 64.
This is the streaming/precomputed-hash signing path used by ACVP test
vectors and protocols that hash the message in a separate step (e.g. when
the message is too large to keep in memory or when $\mu$ is computed by a
different component).  Like \code{mldsa\_sign\_ex()}, the per-signature
randomness \textit{rnd} (32 bytes) is supplied by the caller.
Returns \code{CRYPT\_INVALID\_ARG} if \textit{mulen} is not 64 or
\textit{rndlen} is not 32, and the same status codes as \code{mldsa\_sign\_ex()} otherwise.

\index{mldsa\_compute\_mu()}
\begin{verbatim}
...
Computes the $\mu$ that \code{mldsa\_sign\_ex\_mu()} and \code{mldsa\_verify\_mu()} take:
$\mu = \mathrm{SHAKE256}(tr \,\|\, \mathtt{0x00} \,\|\, \mathit{ctxlen} \,\|\, \mathit{ctx} \,\|\, \mathit{msg}, 64)$
with $tr = \mathrm{SHAKE256}(pk, 64)$.
$tr$ is computed from the public key, so \textit{key} may be a public or a private key.
The output is always 64 bytes. A smaller \textit{*mulen} returns \code{CRYPT\_BUFFER\_OVERFLOW}
with \textit{*mulen} set to the required size, and a \textit{ctxlen} above 255 returns
\code{CRYPT\_INVALID\_ARG}.
...
Verifies against a $\mu$ from \code{mldsa\_compute\_mu()} instead of against the message. It is the
counterpart of \code{mldsa\_sign\_ex\_mu()}. \textit{mulen} must equal 64.
\textit{*stat} and the return value work as in \code{mldsa\_verify()}.
...
Returns the public key, secret key, signature, opt\_rand, and keygen-seed sizes in bytes
for the parameter set specified by \textit{alg}.
Any output pointer may be \code{NULL} if the caller does not need that particular size.
\textit{optrand\_sz} equals the security parameter $n$ (16, 24, or 32 bytes for the
128-, 192-, and 256-bit security levels respectively) and is the length expected by
\code{slhdsa\_sign\_ex()}'s \textit{optrand} argument.
\textit{keygen\_seed\_sz} equals $3n$ and is the length expected by
\code{slhdsa\_make\_key\_from\_seed()}'s \textit{seed} argument.
...
\index{slhdsa\_find\_alg()}\index{slhdsa\_alg\_name()}
\begin{verbatim}
int slhdsa_find_alg(const char *name_or_oid, int *alg);
int slhdsa_alg_name(int alg, const char **name);
\end{verbatim}

\code{slhdsa\_find\_alg()} resolves either a canonical FIPS 205 name (Pure SLH-DSA, e.g.
\texttt{"SLH-DSA-SHA2-128s"}, or HashSLH-DSA, e.g. \texttt{"HashSLH-DSA-SHA2-128s-with-SHA256"})
or a dotted-decimal OID string (e.g. \texttt{"2.16.840.1.101.3.4.3.20"})
to an \code{ltc\_slhdsa\_id} value.  Name comparison is case-insensitive,
ignores \texttt{'-'} and \texttt{'\_'}, and recognises all 24 parameter sets.
\code{slhdsa\_alg\_name()} performs the inverse lookup: given an \code{ltc\_slhdsa\_id}, it returns a pointer
to a static name string (the caller must not free it).
Both return \code{CRYPT\_OK} on success and \code{CRYPT\_INVALID\_ARG} on miss.
...
\index{slhdsa\_make\_key\_from\_seed()}
\begin{verbatim}
int slhdsa_make_key_from_seed(int alg,
                              const unsigned char *seed,
                              unsigned long seedlen,
                              slhdsa_key *key);
\end{verbatim}

Deterministic variant of \code{slhdsa\_make\_key()} corresponding to the
\textit{slh\_keygen\_internal} routine in FIPS 205 \S10.1.
The \textit{seed} input is the concatenation \textit{SK.seed} $\|$ \textit{SK.prf} $\|$ \textit{PK.seed}
and must be exactly $3n$ bytes long, where $n$ is 16, 24, or 32 for the 128-, 192-, and 256-bit
security levels respectively.  This is required to verify FIPS 205 known-answer test vectors.
Returns \code{CRYPT\_INVALID\_ARG} if \textit{seedlen} does not match $3n$.
...
\index{slhdsa\_check\_key()}
\begin{verbatim}
int slhdsa_check_key(const slhdsa_key *key);
\end{verbatim}

Recomputes PK.root from SK.seed and PK.seed and compares it with the copies stored in the private and
in the public key. Since the key structure is public, this is how to check a key object that was built
or changed by hand. Recomputing the root rebuilds the whole top Merkle tree, so it is a separate call
and not part of every signature.
Returns \code{CRYPT\_OK} if the key is consistent, \code{CRYPT\_PK\_INVALID\_TYPE} if a key field is
wrong, and \code{CRYPT\_INVALID\_PACKET} if the key material does not match.

SLH-DSA keys have no separate generation seed. The raw private key already is
SK.seed $\|$ SK.prf $\|$ PK.seed $\|$ PK.root, so its first $3n$ octets can be passed to
\code{slhdsa\_make\_key\_from\_seed()} and storing the expanded form loses nothing.
...
Deterministic variant of \code{slhdsa\_sign()} corresponding to the
\textit{slh\_sign\_internal} routine in FIPS 205 \S10.2.
Instead of drawing the per-signature randomness from a PRNG,
the caller supplies it as \textit{optrand}.
Unlike ML-DSA's \textit{rnd}, the SLH-DSA \textit{optrand} length depends on the parameter set
(16, 24, or 32 bytes for the 128-, 192-, and 256-bit security levels respectively),
matching the \textit{n} parameter; \textit{optrandlen} must equal that value.
Setting \textit{optrand} to the public seed (or all zeros) yields the spec's
``deterministic'' variant; setting it from a CSPRNG yields the ``hedged'' variant
produced by \code{slhdsa\_sign()}.
This is required to verify FIPS 205 known-answer test vectors, which fix \textit{optrand}.
Returns \code{CRYPT\_INVALID\_ARG} if \textit{optrandlen} does not match \textit{n} for
the key's parameter set, and the same status codes as \code{slhdsa\_sign()} otherwise.
...
\hline \texttt{BEGIN CERTIFICATE}            & Public            & Plain                 & \texttt{X.509}    & DH, DSA, ECC, Ed25519, Ed448, ML-DSA, ML-KEM, RSA, SLH-DSA, X25519, X448 \\
...
\hline \texttt{BEGIN ENCRYPTED PRIVATE KEY}  & Private           & Encrypted             & \texttt{PKCS \#8} & DH, DSA, ECC, Ed25519, Ed448, ML-DSA, ML-KEM, RSA, SLH-DSA, X25519, X448 \\
\hline \texttt{BEGIN PRIVATE KEY}            & Private           & Plain                 & \texttt{PKCS \#8} & DH, DSA, ECC, Ed25519, Ed448, ML-DSA, ML-KEM, RSA, SLH-DSA, X25519, X448 \\
\hline \texttt{BEGIN PUBLIC KEY}             & Public            & Plain                 & \texttt{X.509\footnote{Specifically, SubjectPublicKeyInfo}} & DH, DSA, ECC, Ed25519, Ed448, ML-DSA, ML-KEM, RSA, SLH-DSA, X25519, X448 \\
